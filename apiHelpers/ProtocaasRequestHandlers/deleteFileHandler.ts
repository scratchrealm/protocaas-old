import { DeleteFileRequest, DeleteFileResponse } from "../../src/types/ProtocaasRequest";
import getProject from '../getProject';
import { getMongoClient } from "../getMongoClient";
import getWorkspace from '../getWorkspace';
import { userCanDeleteFile } from "../permissions";
import removeIdField from "../removeIdField";
import { isProtocaasDataBlob, isProtocaasFile, isProtocaasJob, ProtocaasDataBlob, ProtocaasFile, ProtocaasJob } from "../../src/types/protocaas-types";
...
export const cleanupProject = async (projectId: string) => {
    ...
    let files = removeIdField(await filesCollection.find({projectId}).toArray()) as ProtocaasFile[]
    files.forEach(file => {
    ...
    let jobs = removeIdField(await jobsCollection.find({projectId}).toArray()) as ProtocaasJob[]
    jobs.forEach(job => {
    ...
    let dataBlobs = removeIdField(await dataBlobsCollection.find({projectId}).toArray()) as ProtocaasDataBlob[]
