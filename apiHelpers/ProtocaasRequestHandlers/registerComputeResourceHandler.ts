import { RegisterComputeResourceRequest, RegisterComputeResourceResponse } from "../../src/types/ProtocaasRequest";
import { ProtocaasComputeResource } from "../../src/types/protocaas-types";
...
const registerComputeResourceHandler = async (request: RegisterComputeResourceRequest, o: {verifiedClientId?: string, verifiedUserId?: string}): Promise<RegisterComputeResourceResponse> => {
    ...
    if (cr) {
        ...
        const x: ProtocaasComputeResource = {
            computeResourceId,
            ownerId: userId,
            name: request.name,
            timestampCreated: Date.now() / 1000,
