import fs from 'fs'
import postProtocaasRequestFromComputeResource from "./postProtocaasRequestFromComputeResource"
import PubsubClient from './PubsubClient'
import JobManager from "./JobManager"
import { GetPubsubSubscriptionRequest, GetJobsRequest, SetComputeResourceSpecRequest } from "./types/ProtocaasRequest"
...
class JobExecutor {
    ...
    constructor(private a: { dir: string }) {
        // read computeResourceId from .protocaas-compute-resource-node.yaml in dir directory
        ...
        if (container_method === 'none') {
            ...
            if (process.env.PROTOCAAS_DANGEROUS_CONTAINER_METHOD_NONE !== 'true') {
                throw Error('Container method is set to none. Set environment variable PROTOCAAS_DANGEROUS_CONTAINER_METHOD_NONE to true to allow this.')
            }
        }
    ...
    async start() {
        ...
        if (fs.existsSync(specJsonFname)) {
            ...
            const respSpec = await this._postProtocaasRequest(reqSpec)
            if (!respSpec) {
        ...
        const respPubsub = await this._postProtocaasRequest(reqPubsub)
    ...
    private async _processPendingJobs() {
        ...
        const resp = await this._postProtocaasRequest(req)
        if (resp) {
    ...
    private async _postProtocaasRequest(req: any): Promise<any> {
        return await postProtocaasRequestFromComputeResource(req, {
            computeResourceId: process.env.COMPUTE_RESOURCE_ID,
