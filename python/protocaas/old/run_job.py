...
def run_job():
    from .ProtocaasPluginTypes import ProtocaasPlugin, ProtocaasPluginContext, ProtocaasProcessingTool, ProtocaasProcessingToolContext, InputFile, OutputFile
    ...
    processing_tools: list[ProtocaasProcessingTool] = []
    class ProtocaasPluginContextImpl(ProtocaasPluginContext):
        def __init__(self):
            pass
        def register_processing_tool(self, tool: ProtocaasPlugin):
            processing_tools.append(tool)
    plugin_context = ProtocaasPluginContextImpl()
    plugin_package_names = ['protocaas']
    for plugin_package_name in plugin_package_names:
        ...
        for attr_name in dir(module):
            ...
            if inspect.isclass(X) and issubclass(X, ProtocaasPlugin):
    ...
    class ProtocaasProcessingToolContextImpl(ProtocaasProcessingToolContext):
        ...
        def upload_output_file(self, output_file: OutputFile, path: str):
            ...
            s3.upload_file(path, OUTPUT_BUCKET, f'protocaas-dev/{random_output_id}/{basename}')
            # for now we hard-code neurosift.org
            url = f'{OUTPUT_BUCKET_BASE_URL}/protocaas-dev/{random_output_id}/{basename}'
    ...
    tool.run(ProtocaasProcessingToolContextImpl())
